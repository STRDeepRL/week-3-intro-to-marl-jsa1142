        ...
        for team, agent_actions in actions.items():
            for idx, action in enumerate(agent_actions):
                ...
                if (agent.name in self.policies_map) and not agent.terminated:
                    ...
                    agent_reward, agent_terminated, agent_info = (
                        reward[agent_index],
                        terminated[agent_index],
                        info[agent.name],
                    )
                    agent_reward, agent_terminated, agent_info = self.policies_map[agent.name].custom_handle_steps(
                        agent,
                        agent_index,
                        action,
                        ...
                        agent_terminated,
                        agent_info,
                        self.reward_schemes[agent.name],
                        self.training_scheme,
                    )
                    reward[agent_index], terminated[agent_index], info[agent.name] = (
                        agent_reward,
                        agent_terminated,
                        agent_info,
                    )
